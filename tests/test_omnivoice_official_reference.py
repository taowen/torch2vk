...
class OmniVoiceOfficialReferenceTest(unittest.TestCase):
    def test_provider_construction_does_not_import_official_package(self) -> None:
        ...
        self.assertEqual(provider.provider_id, "omnivoice_safetensor.official_generate.v2")
    ...
    def test_provider_maps_final_audio_tokens_boundary(self) -> None:
        final_tokens = LogicalTensor(
            "generate.final.audio_tokens",
            TensorSpec(dtype="int32", shape=(2,)),
            pytorch_probe=PyTorchProbe(kind="manual", source="generate.final.audio_tokens"),
        ...
        with (
            ...
            patch(
                "torch2vk.models.omnivoice_safetensor.reference.capture_official_omnivoice_trace",
                return_value=ReferenceTrace(
                    tokens={
                        "generate.final.audio_tokens": torch.tensor([3, 4], dtype=torch.int32)
                    }
                ),
            ),
        ):
            provider = omnivoice_official_reference_provider()
            artifacts = provider.ensure(
                tensors=(final_tokens,),
                inputs={"text": "hello", "target_steps": 1},
                cache=ArtifactCache(Path(cache_dir)),
            )

        self.assertTrue(
            artifacts["generate.final.audio_tokens"].equal(
    ...
    def test_official_capture_smoke(self) -> None:
        ...
        self.assertIn("generate.final.audio_tokens", trace.tokens)
        self.assertGreater(trace.tensors["output.wav"].numel(), 0)
        self.assertEqual(trace.tensors["output.wav_pcm16"].dtype, torch.int16)
        self.assertEqual(trace.tokens["generate.final.audio_tokens"].dtype, torch.int32)
