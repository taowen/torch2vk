...
任何需要引用 tensor 的地方都直接持有 `LogicalTensor` 对象。`LogicalTensor.name` 只用于稳定报告、
debug、artifact 路径和重复声明检查，不作为 runtime 查找 tensor 的 key。
...
```text
tensors/ = 点
  声明 LogicalTensor：name/spec/role/memory/lifetime/source/semantic/probe/compare
...
`WeightSource` 只是 metadata。实际打开 checkpoint、上传权重、查找输入、分配 buffer 都由 RuntimeSession 完成。
...
8. RuntimeSession readback 对应 candidate LogicalTensor 的当前 buffer
