...
```python
rt.debug_materialization(tensor)
```
...
Frame 内:
  ...
  RuntimeSession 记录每次 dispatch 的 reads/writes/writer/frame
...
`LogicalTensor.name` 是一次 invocation 内的模型语义名。现在的设计是执行多少次就创建多少份
`LogicalTensor` tree，因此动态执行上下文由本次 invocation 的 frame name 和这份 tensor tree 的 name 一起表达。
...
frame name 必须包含足够的 invocation 身份，例如：

```text
text_llm.decode.audio_token_005.cond
text_llm.decode.audio_token_005.uncond
qwen3_asr.audio_tower.chunk_000
```

对应 tensor name 可以继续使用同一份 invocation tree 内的语义名：

```text
text_llm.decode.audio_token_005.cond.layer.03.output
qwen3_asr.audio_tower.chunk_000.last_hidden_state
```

artifact key 直接由 `frame/tensor.name` 组成，不再拼接单独的 scope map。
...
3. 同一个 logical name 在不同 invocation 下需要不同值时，由模型目录声明不同 `LogicalTensor` 实例；
...
ACTIVATION/OUTPUT 等上游写出值
  必须已经在当前 frame 或可见上游 frame 中被写出
  未写先读时报错
```
