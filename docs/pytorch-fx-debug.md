...
这类 reference 仍然通过同一个 `ReferenceSpec.output_bindings` 对拍。区别只是 expected 由调用方计算，而不是
`ExportedProgramReference.execute()` 计算。
...
2. `reference_specs.py`：记录 reference input/output key 到 tensor field path 的映射；
...
for step in generation:
    ...
    run PyTorch reference subgraph(s) with its own state
    compare outputs through ReferenceSpec
