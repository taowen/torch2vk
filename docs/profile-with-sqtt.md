...
`attribution.jsonl` 是 join 后的结果。`pipeline-attribution.json` 和
`source-isa-sqtt-hotspots*.json` 由 `src/torch2vk/sqtt` 读取 RGP/SQTT stream、driver disasm/source
map 后生成。
...
scripts/profile-sqtt.sh \
  --root .cache/torch2vk/sqtt/qwen3-logits \
  --capture-filter 'frame=qwen3_asr.text_decode.0000;shader=qwen3_asr_text_linear_nobias_f32;dispatch=123' \
  -- \
...
7. 运行用户命令。
8. 复制 RGP 到 `<root>/rgp/`，生成 `attribution.jsonl` 和 source/ISA 热点报告。

如果只知道 shader，可以先用 `--shader qwen3_asr_text_linear_nobias_f32`。如果 RGP 仍然太多，
先跑普通 profiler 找到目标 `dispatch_index`，再用完整 `--capture-filter` 精确到单个 dispatch。
...
6. `src/torch2vk/sqtt` 解析 RGP/SQTT stream，并按 driver source map 输出 source/ISA 热点报告。
