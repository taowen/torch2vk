...
```bash
scripts/profile-sqtt.sh --root .cache/torch2vk/sqtt/qwen3-decode -- \
  uv run pytest tests/test_qwen3_asr.py -k decode_record
```
...
4. 设置 `AGENTORCH_RADV_DRIVER_ARTIFACTS_DIR=<root>/driver`。
5. 设置 `TORCH2VK_PROFILE_RUN_DIR=<root>`，让 runtime manifest 写到同一个目录。
6. 运行用户命令。
7. 复制新增 RGP 到 `<root>/rgp/`。
...
4. `dispatches.jsonl` 缺失或为空。
5. `driver/capture-sequence.jsonl` 缺失或为空。
6. `driver/dispatch-sequence.jsonl` 缺失或为空。
7. RGP 文件缺失或为空。
8. driver label payload 无法解析，或无法按 frame/shader/dispatch join 到 manifest。
9. pipeline hash 能 join，但 pipeline identity 或 SPIR-V hash 对不上。
10. manifest 中出现 `phase != "record"` 的 dispatch。
...
1. 让 `scripts/profile-sqtt.sh` 同时设置 `TORCH2VK_PROFILE_RUN_DIR=<root>`。
2. 实现普通 profiler 的 manifest-only 写出。
3. record 模式的 `RuntimeSession.dispatch()` 录制 debug label。
4. 写 postprocess：读 manifest、driver JSONL、RGP 路径，输出 `attribution.jsonl`。
5. 如果 profile run 进入 replay，直接报错，提示 SQTT 只支持 record 模式。
