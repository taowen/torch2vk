...
torch2vk 的 reference 框架为此维护了一份完整的 PyTorch 状态：
- DynamicCache 对象（保持 KV cache 的 PyTorch 版本）
- token 列表和 stopped 标志

每个 decode step 同时推进 Vulkan 和 PyTorch 两边的状态，确保两者的 KV cache 内容始终一致，而不只是最终输出一致。
