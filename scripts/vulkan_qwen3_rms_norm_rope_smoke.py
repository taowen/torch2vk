#!/usr/bin/env python3
"""Run the copied Qwen3 fused RMS norm + RoPE shader through Vulkan."""
...
from torch2vk.models.qwen3_safetensor.shaders.rms_norm_mul_rope_q_f32 import (
...
def main() -> int:
    variant = RMS_NORM_MUL_ROPE_Q_F32
    batch = 1
    steps = 1
    heads = 1
    head_dim = 128
    ...
    expected = _rms_norm_reference(x=x, weight=weight, head_dim=head_dim)
    tensors = {
        ...
        "output": activation_tensor(
            "output",
            dtype="float32",
            shape=(batch, steps, heads, head_dim),
        ),
    }
    ...
    try:
        ...
        row_indices_buffer.write(struct.pack("<q", 0))
        ...
        try:
            ...
            print(f"qwen3_rms_norm_rope_dispatch=ok values={actual[:4]}")
        finally:
            fence.close()
            command_pool.close()
            pipeline.close()
...
def _rms_norm_reference(
    *,
    ...
    mean = sum(value * value for value in x) / head_dim
    scale = 1.0 / math.sqrt(mean + 1.0e-6)
    return tuple(value * scale * rounded_weight[index] for index, value in enumerate(x))
