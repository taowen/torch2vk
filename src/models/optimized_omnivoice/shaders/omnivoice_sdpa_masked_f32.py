"""Generated shader: omnivoice_sdpa_masked_f32."""
    ...
    contract=ShaderContract(
        ...
        fields=(
            ...
            TensorFieldSpec(
                ...
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=('B', 1, 'T', 'S',)),
            ),
            TensorFieldSpec(
                name='output',
...
layout(set = 0, binding = 3) buffer restrict readonly MaskBuffer { float16_t mask[]; };
...
void main() {
    ...
    if (float(mask[mask_row_base]) < -1.0e30) {
        if (valid0) {
    ...
    while (valid_cols > 0u && float(mask[mask_row_base + valid_cols - 1u]) < -1.0e30) {
        --valid_cols;
    }
