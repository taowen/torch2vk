"""Fused RMSNorm, RoPE, and transpose for Qwen3 decode attention."""
...
RMS_NORM_ROPE_TRANSPOSE_F16 = ShaderVariant(
    name="rms_norm_rope_transpose_f16",
    family="optimized_omnivoice",
    contract=ShaderContract(
        class_name="OptimizedOmniVoiceRmsNormRopeTransposeF16Program",
        shader_name="rms_norm_rope_transpose_f16",
        fields=(
            TensorFieldSpec(
                name="x",
...
void main() {
    ...
    for (uint h = tid; h < pc.D; h += 256u) {
        const float v = float(x[x_base + h]);
    ...
    const float16_t norm_value = float16_t(float(x[x_base + d]) * inv_rms * weight[d]);
    const float16_t norm_rotated = float16_t(sign_value * float(x[x_base + rotated_d]) * inv_rms * weight[rotated_d]);
    const uint rope_idx = (b * pc.T + t) * pc.D + d;
    const float16_t left = float16_t(norm_value * cos_values[rope_idx]);
    const float16_t right = float16_t(norm_rotated * sin_values[rope_idx]);
