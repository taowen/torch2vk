"""Generated shader: linear_bias_bf16w_bf16b_f32."""
...
LINEAR_BIAS_BF16W_BF16B_F32 = ShaderVariant(
    name='linear_bias_bf16w_bf16b_f32',
    family='export',
    contract=ShaderContract(
        class_name='ExportLinearBiasBf16WeightBf16BiasProgram',
        shader_name='linear_bias_bf16w_bf16b_f32',
        fields=(
            TensorFieldSpec(
                name='x',
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=('X0', 'X1',)),
            ),
            TensorFieldSpec(
                name='weight',
                io_kind=IOKind.INPUT,
            ...
            TensorFieldSpec(
                ...
                role='output',
                contract=TensorContract(dtype='float16', shape=('Y0', 'Y1',)),
            ),
        ),
        push_constants=PushConstantSpec(
            ...
            fields=(
                ...
                PushConstantFieldSpec('K', PushConstantType.UINT32, 4, 'X1', dynamic=False),
                PushConstantFieldSpec('N', PushConstantType.UINT32, 8, 'W0', dynamic=False),
            ),
        ),
        params_buffer=None,
        dispatch=(ceil_div('X0', 16), ceil_div('W0', 16), 1),
    ),
...
shared float16_t tile_x[16 * 32]; shared bfloat16_t tile_w[32 * 16];
void main() {
    ...
    if (row < pc.M && col < pc.N) { output_values[row * pc.N + col] = float16_t(acc + float(bias[col])); }
}
""",
)
