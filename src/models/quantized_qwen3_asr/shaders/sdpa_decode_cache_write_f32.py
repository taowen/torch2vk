"""Decode SDPA fused with single-token f32 KV cache write."""
...
layout(set = 0, binding = 1) buffer restrict readonly NewKBuffer { float new_k[]; };
layout(set = 0, binding = 2) buffer restrict readonly NewVBuffer { float new_v[]; };
...
void main() {
    ...
    const float new_k_value = valid_dim ? new_k[new_offset] : 0.0;
    const float new_v_value = valid_dim ? new_v[new_offset] : 0.0;
