"""Generated shader: linear_nobias_q4_k_f32_act_f32."""
...
LINEAR_NOBIAS_Q4_K_F32_ACT_F32 = ShaderVariant(
    name='linear_nobias_q4_k_f32_act_f32',
    family='export',
    contract=ShaderContract(
        class_name='ExportLinearNobiasQ4KProgram',
        shader_name='linear_nobias_q4_k_f32_act_f32',
        fields=(
            TensorFieldSpec(
                name='x',
...
shared float shared_stage[WARPS * STAGE_SIZE];
...
void load_x_tile(uint local_id, uint m_base, uint k_base) {
    for (uint i = local_id; i < TILE_MK_LOADS; i += 128u) {
        ...
        if (m < pc.M && k + 7u < pc.K) {
            ...
            const vec4 values0 = x4[input_base >> 2u];
            const vec4 values1 = x4[(input_base >> 2u) + 1u];
...
void main() {
    ...
    coopmat<float, gl_ScopeSubgroup, 16, 16, gl_MatrixUseAccumulator> sums[CMS_PER_ROW * CMS_PER_COL];
    ...
    [[unroll]] for (uint cm_row = 0u; cm_row < CMS_PER_ROW; ++cm_row) {
        [[unroll]] for (uint cm_col = 0u; cm_col < CMS_PER_COL; ++cm_col) {
            ...
            coopmat<float, gl_ScopeSubgroup, 16, 16, gl_MatrixUseAccumulator> output_tile =
                coopmat<float, gl_ScopeSubgroup, 16, 16, gl_MatrixUseAccumulator>(sums[sum_index]);
