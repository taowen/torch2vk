"""Generated shader: decode_layer_mul_broadcast_11."""
...
DECODE_LAYER_MUL_BROADCAST_11 = ShaderVariant(
    name='decode_layer_mul_broadcast_11',
    family='export',
    contract=ShaderContract(
        class_name='ExportMulBroadcastProgram',
        shader_name='decode_layer_mul_broadcast_11',
        fields=(
            TensorFieldSpec(
                name='x',
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=(1, 1, 'H', 'D',)),
            ),
            TensorFieldSpec(
                name='y',
                io_kind=IOKind.INPUT,
...
void main() {
    ...
    const uint x_idx = c2 * pc.D3 + c3 * 1u;
    const uint y_idx = c2 * 1u;
