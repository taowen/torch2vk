"""Prefill causal SDPA fused with f32 KV cache write."""
...
SDPA_CAUSAL_CACHE_WRITE_F32 = ShaderVariant(
    ...
    contract=ShaderContract(
        class_name="SdpaCausalCacheWriteF32Program",
        ...
        fields=(
            TensorFieldSpec(
                ...
                IOKind.INPUT,
                "input",
                TensorContract(dtype="float32", shape=("B", "NH", "T", "D")),
            ),
            TensorFieldSpec(
                "k",
                IOKind.INPUT,
                "input",
                TensorContract(dtype="float32", shape=("B", "NK", "T", "D")),
            ),
            TensorFieldSpec(
                "v",
                IOKind.INPUT,
...
void main() {
    ...
    if (cache_writer && dst_t < pc.S) {
        ...
        const uint v_row_base = k_base + row * pc.D;
        if (valid0) {
            k_cache[cache_row_base + dim0] = k[k_row_base + dim0];
            v_cache[cache_row_base + dim0] = v[v_row_base + dim0];
        }
        if (valid1) {
            k_cache[cache_row_base + dim1] = k[k_row_base + dim1];
            v_cache[cache_row_base + dim1] = v[v_row_base + dim1];
        }
    }
    ...
    for (uint col = 0u; col <= row && col < pc.S; ++col) {
        ...
        const uint v_base = k_base + col * pc.D;
        ...
        if (valid0) {
            acc0 = acc0 * old_scale + score_scale * v[v_base + dim0];
        }
        if (valid1) {
            acc1 = acc1 * old_scale + score_scale * v[v_base + dim1];
        }
