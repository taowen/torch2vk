...
QWEN3_ASR_TEXT_KV_CACHE_WRITE_DECODE_F32 = ShaderVariant(
    name="qwen3_asr_text_kv_cache_write_decode_f32",
    family="qwen3_asr.text",
    contract=ShaderContract(
        class_name="Qwen3AsrTextKvCacheWriteDecodeF32Program",
        shader_name="qwen3_asr_text_kv_cache_write_decode_f32",
        fields=(
            TensorFieldSpec(
                name="k",
                io_kind=IOKind.INPUT,
...
layout(set = 0, binding = 2) buffer restrict readonly CachePositionBuffer {
    int64_t cache_position[];
};

layout(set = 0, binding = 3) buffer restrict KeyCacheBuffer {
    float key_cache[];
};

layout(set = 0, binding = 4) buffer restrict ValueCacheBuffer {
    float value_cache[];
};
