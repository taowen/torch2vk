"""Qwen3-ASR text RMS normalization shader (no bias, no mean subtraction)."""
...
QWEN3_ASR_TEXT_RMS_NORM_F32 = ShaderVariant(
    name="qwen3_asr_text_rms_norm_f32",
    family="qwen3_asr.text",
    contract=ShaderContract(
        class_name="Qwen3AsrTextRmsNormF32Program",
        shader_name="qwen3_asr_text_rms_norm_f32",
        fields=(
            TensorFieldSpec(
                name="x",
...
void main() {
    ...
    for (uint h = tid; h < pc.H; h += 256u) {
        output_values[row * pc.H + h] = x[row * pc.H + h] * inv_rms * bf16_to_f32(weight[h]);
    }
}
""".lstrip(),
)
