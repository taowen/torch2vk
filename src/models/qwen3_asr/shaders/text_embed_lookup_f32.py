"""Qwen3-ASR single-token embedding lookup for decode."""
...
QWEN3_ASR_TEXT_EMBED_LOOKUP_F32 = ShaderVariant(
    name="qwen3_asr_text_embed_lookup_f32",
    family="qwen3_asr.text",
    contract=ShaderContract(
        class_name="Qwen3AsrTextEmbedLookupF32Program",
        shader_name="qwen3_asr_text_embed_lookup_f32",
        fields=(
            TensorFieldSpec(
                name="input_ids",
...
layout(set = 0, binding = 1) buffer restrict readonly EmbedBuffer {
    uint16_t embed_tokens[];
};
...
void main() {
    ...
    // input_ids is int64 stored as two int32s (little-endian)
