"""Fused Q/K Q4_K decode matvec for Qwen3."""
...
LINEAR_NOBIAS_Q4_K_QK_MATVEC_F32 = ShaderVariant(
    name="linear_nobias_q4_k_qk_matvec_f32",
    family="optimized_qwen3",
    contract=ShaderContract(
        class_name="LinearNobiasQ4KQKMatvecProgram",
        shader_name="linear_nobias_q4_k_qk_matvec_f32",
        fields=(
            TensorFieldSpec(
                "x",
                IOKind.INPUT,
                "input",
                TensorContract(dtype="float16", shape=("X0", "X1", "K")),
            ),
            TensorFieldSpec(
                "q_weight",
                IOKind.INPUT,
            ...
            TensorFieldSpec(
                ...
                IOKind.OUTPUT,
                "output",
                TensorContract(dtype="float16", shape=("X0", "X1", "NQ")),
            ),
            TensorFieldSpec(
                "k_output",
                IOKind.OUTPUT,
                "output",
                TensorContract(dtype="float16", shape=("X0", "X1", "NK")),
            ),
        ),
        ...
        dispatch=(ceil_div(add("NQ", "NK"), 2), mul("X0", "X1"), 1),
    ),
...
void main() {
    ...
    const uint col = gl_WorkGroupID.x * 2u + subgroup;
    if (row >= pc.M) {
        return;
    }
    if (col >= total_n) {
