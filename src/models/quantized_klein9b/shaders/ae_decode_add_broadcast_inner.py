"""Generated shader: ae_decode_add_broadcast_inner."""
...
AE_DECODE_ADD_BROADCAST_INNER = ShaderVariant(
    name='ae_decode_add_broadcast_inner',
    family='export',
    contract=ShaderContract(
        class_name='ExportAddBroadcastInnerProgram',
        shader_name='ae_decode_add_broadcast_inner',
        fields=(
            TensorFieldSpec(
                name='x',
            ...
            TensorFieldSpec(
                ...
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float32', shape=(1, 'T', 1, 1,)),
            ),
            TensorFieldSpec(
                name='output',
        ...
        push_constants=PushConstantSpec(
            ...
            fields=(
                ...
                PushConstantFieldSpec('REPEAT', PushConstantType.UINT32, 8, 'D', dynamic=False),
            ),
        ),
        params_buffer=None,
...
void main() {
    ...
    if (idx < pc.N) {
        ...
        output_values[idx] = float16_t(float(x[idx]) + float(y[broadcast_idx]));
    }
}
""",
)
