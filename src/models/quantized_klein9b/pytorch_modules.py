...
class FluxPeJoinModule(nn.Module):
    def forward(self, pe_ctx: Tensor, pe_x: Tensor) -> Tensor:
...
class Flux2(nn.Module):
    def __init__(self, params: Klein9BParams):
        super().__init__()
...
class DoubleStreamBlock(nn.Module):
    ...
    def _prepare_qkv(self, img, txt, pe, pe_ctx, mod_img, mod_txt):
        ...
        img_q_raw, img_k_raw, img_v = rearrange(
            img_qkv,
            "B L (K H D) -> K B H L D",
            K=3,
            H=self.num_heads,
        ...
        txt_q_raw, txt_k_raw, txt_v = rearrange(
            txt_qkv,
            "B L (K H D) -> K B H L D",
            K=3,
            H=self.num_heads,
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x: Tensor):
        return F.rms_norm(x, (self.scale.shape[0],), self.scale, eps=1e-6)
...
class FluxDoubleBlockModule(DoubleStreamBlock):
    ...
    def forward(
        ...
        pe: Tensor,
        pe_ctx: Tensor,
        img_mod1_shift: Tensor,
        img_mod1_scale: Tensor,
        img_mod1_gate: Tensor,
        img_mod2_shift: Tensor,
        img_mod2_scale: Tensor,
        img_mod2_gate: Tensor,
        txt_mod1_shift: Tensor,
        txt_mod1_scale: Tensor,
        txt_mod1_gate: Tensor,
        txt_mod2_shift: Tensor,
        txt_mod2_scale: Tensor,
        txt_mod2_gate: Tensor,
    ) -> tuple[Tensor, ...]:
        img_mod = (
        ...
        return DoubleStreamBlock.forward_block(
            self,
            img,
            txt,
            pe,
...
class FluxSingleBlockModule(SingleStreamBlock):
    def __init__(self, block: SingleStreamBlock) -> None:
        nn.Module.__init__(self)
    ...
    def forward(
        ...
        hidden_states = SingleStreamBlock.forward_block(
            self,
            hidden_states,
            pe,
...
class FluxFinalLayerModule(nn.Module):
    ...
    def forward(self, hidden_states: Tensor, vec: Tensor) -> Tensor:
