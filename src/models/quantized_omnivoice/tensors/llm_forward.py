...
from torch2vk.vulkan.types import CONTIGUOUS_LAYOUT, TensorSpec, q4_k_words_layout
...
def _declare_tensor(
    *,
    ...
    if role is TensorRole.WEIGHT and spec.dtype == "float32" and len(spec.shape) == 2:
        out_features, in_features = spec.shape
        if not isinstance(out_features, int) or not isinstance(in_features, int):
            raise ValueError(f"Q4_K weight requires concrete shape, got {spec.shape}")
        layout = q4_k_words_layout(logical_k=in_features)
        if in_features % layout.block_size != 0:
            raise ValueError(f"Q4_K weight requires K divisible by {layout.block_size}, got {in_features}")
        spec = TensorSpec(
            dtype="uint32",
            shape=(out_features, in_features // layout.block_size * layout.words_per_block),
        )
