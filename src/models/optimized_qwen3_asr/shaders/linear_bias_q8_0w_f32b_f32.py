"""Generated shader: linear_bias_q8_0w_f32b_f32."""
...
LINEAR_BIAS_Q8_0W_F32B_F32 = ShaderVariant(
    name='linear_bias_q8_0w_f32b_f32',
    family='export',
    contract=ShaderContract(
        class_name='ExportLinearBiasQ8_0WeightF32BiasProgram',
        shader_name='linear_bias_q8_0w_f32b_f32',
        fields=(
            TensorFieldSpec(
                name='x',
...
void main() {
    ...
    for (uint i = lane; i < OUT_SIZE; i += 64u) {
        ...
        if (n < pc.N) {
            if (m0 < pc.M) {
                output_values[m0 * pc.N + n] = float16_t(shared_out0[i] + float(bias[n]));
            }
            if (m1 < pc.M) {
                output_values[m1 * pc.N + n] = float16_t(shared_out1[i] + float(bias[n]));
            }
        }
    }
}
