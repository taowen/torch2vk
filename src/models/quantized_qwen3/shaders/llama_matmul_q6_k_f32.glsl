...
layout(binding = 0) readonly buffer A { block_q6_K data_a[]; };

layout(binding = 0) readonly buffer A_PACKED16 { block_q6_K_packed16 data_a_packed16[]; };
...
#line 1 "/var/home/taowen/projects/agentorch/third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_funcs.glsl"
 void load_a_to_shmem(const uint pos_a, const uint row, const uint col, const uint idx_m, const uint block, const uint end_k) {
            ...
            const uint idx = pos_a + col * p.stride_a / 2 + row;
            const uint buf_idx = col * (BK / 2 + 4) + row * 2 / 2;
...
void main() {
    ...
    const uint loadr_a = gl_LocalInvocationID.x % (BK / 2 / 1);
    const uint loadc_a = gl_LocalInvocationID.x / (BK / 2 / 1);
    const uint loadr_b = gl_LocalInvocationID.x % (BK / 8 / 1);
    const uint loadc_b = gl_LocalInvocationID.x / (BK / 8 / 1);

    const uint loadstride_a = gl_WorkGroupSize.x * 2 * 1 / BK;
    ...
    uint pos_a =
        ...
        batch_idx_a * (p.batch_stride_a / 2) +

        (ir * BM * p.stride_a + start_k) / 2;
