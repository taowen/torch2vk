"""Optimized prefill causal SDPA fused with KV cache write."""
...
SDPA_CAUSAL_CACHE_WRITE_F32 = ShaderVariant(
    ...
    contract=ShaderContract(
        class_name='QuantizedQwen3SdpaCausalCacheWriteProgram',
        ...
        fields=(
            ...
            TensorFieldSpec(
                ...
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=('B', 'NK', 'T', 'D',)),
            ),
            TensorFieldSpec(
                name='v',
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=('B', 'T', 'NK', 'D',)),
            ),
            TensorFieldSpec(
                name='k_cache',
        ...
        push_constants=PushConstantSpec(
            ...
            fields=(
                ...
                PushConstantFieldSpec('NH', PushConstantType.UINT32, 4, 'NH', dynamic=False),
                PushConstantFieldSpec('NK', PushConstantType.UINT32, 8, 'NK', dynamic=False),
                PushConstantFieldSpec('T', PushConstantType.UINT32, 12, 'T', dynamic=False),
                PushConstantFieldSpec('S', PushConstantType.UINT32, 16, 'S', dynamic=False),
                PushConstantFieldSpec('D', PushConstantType.UINT32, 20, 'D', dynamic=False),
            ),
        ),
        params_buffer=None,
...
void main() {
    ...
    for (uint col = 0u; col <= row && col < pc.S; ++col) {
        ...
        if (valid0) {
            acc0 = acc0 * old_scale + score_scale * float(v[v_base + dim0]);
        }
        if (valid1) {
            acc1 = acc1 * old_scale + score_scale * float(v[v_base + dim1]);
        }
