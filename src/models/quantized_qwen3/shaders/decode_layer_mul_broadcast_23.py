"""Generated shader: decode_layer_mul_broadcast_23."""
...
DECODE_LAYER_MUL_BROADCAST_23 = ShaderVariant(
    name='decode_layer_mul_broadcast_23',
    family='export',
    contract=ShaderContract(
        class_name='ExportMulBroadcastProgram',
        shader_name='decode_layer_mul_broadcast_23',
        fields=(
            TensorFieldSpec(
                name='x',
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=(1, 'T', 1, 'D',)),
            ),
            TensorFieldSpec(
                name='y',
                io_kind=IOKind.INPUT,
                role='input',
                contract=TensorContract(dtype='float16', shape=(1, 1, 1, 'D',)),
            ),
            TensorFieldSpec(
                name='output',
...
void main() {
    ...
    const uint x_idx = c1 * 1u * pc.D3 + c3 * 1u;
    const uint y_idx = c3 * 1u;
