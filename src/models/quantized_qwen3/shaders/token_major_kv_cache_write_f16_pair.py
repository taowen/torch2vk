"""Write one token-major float16 K/V tensor into two token-major caches."""
...
TOKEN_MAJOR_KV_CACHE_WRITE_F16_PAIR = ShaderVariant(
    name="token_major_kv_cache_write_f16_pair",
    family="quantized_qwen3",
    contract=ShaderContract(
        class_name="QuantizedQwen3TokenMajorKvCacheWriteF16PairProgram",
        shader_name="token_major_kv_cache_write_f16_pair",
        fields=(
            TensorFieldSpec(
                "cache_a",
                IOKind.INOUT,
            ...
            TensorFieldSpec(
                ...
                IOKind.INPUT,
                "input",
                TensorContract(dtype="float16", shape=("B", "T", "H", "D")),
            ),
        ),
        push_constants=PushConstantSpec(
            ...
            fields=(
                ...
                PushConstantFieldSpec("T", PushConstantType.UINT32, 20, "T"),
            ),
        ),
...
layout(set = 0, binding = 2) buffer restrict readonly CachePositionBuffer { int64_t cache_position[]; };
layout(set = 0, binding = 3) buffer restrict readonly SrcBuffer { float16_t src[]; };
layout(push_constant) uniform PushConstants { uint B; uint SA; uint SB; uint H; uint D; uint T; } pc;
...
void main() {
    ...
    const float16_t value = src[src_idx];
    if (dst_t < pc.SA) {
