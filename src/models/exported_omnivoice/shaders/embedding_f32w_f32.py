"""Generated shader: embedding_f32w_f32."""
...
EMBEDDING_F32W_F32 = ShaderVariant(
    name='embedding_f32w_f32',
    family='export',
    contract=ShaderContract(
        class_name='ExportEmbeddingF32WeightProgram',
        shader_name='embedding_f32w_f32',
        fields=(
            TensorFieldSpec(
                name='weight',
        ...
        push_constants=PushConstantSpec(
            size=8,
            fields=(
                PushConstantFieldSpec('num_indices', PushConstantType.UINT32, 0, 69, dynamic=False),
                PushConstantFieldSpec('embedding_dim', PushConstantType.UINT32, 4, 64, dynamic=False),
            ),
        ),
        params_buffer=None,
        dispatch=(ceil_div(4416, 256), 1, 1),
    ),
...
void main() {
    ...
    output_values[idx] = float16_t(float(weight[uint(token_id) * pc.embedding_dim + dim_idx]));
}
""",
)
