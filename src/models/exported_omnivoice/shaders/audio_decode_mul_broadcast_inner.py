"""Generated shader: audio_decode_mul_broadcast_inner."""
...
AUDIO_DECODE_MUL_BROADCAST_INNER = ShaderVariant(
    name='audio_decode_mul_broadcast_inner',
    family='export',
    contract=ShaderContract(
        class_name='ExportMulBroadcastInnerProgram',
        shader_name='audio_decode_mul_broadcast_inner',
        fields=(
            TensorFieldSpec(
                name='x',
...
void main() {
    ...
    if (idx < pc.N) {
        ...
        output_values[idx] = float16_t(float(x[broadcast_idx]) * float(y[idx]));
    }
}
""",
)
