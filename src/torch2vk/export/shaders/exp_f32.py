...
void main() {
    ...
    if (idx < pc.N) { output_values[idx] = {{STORE_EXP}}; }
}
"""


def make_exp_variant(node: Node, activation_dtype: str = "float32") -> ShaderVariant | None:
...
def _source(activation_dtype: str) -> str:
    return (
        ...
        .replace("{{STORE_EXP}}", activation_store("exp(float(x[idx]))", activation_dtype))
    )
