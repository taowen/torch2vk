...
ATEN_EMBEDDING_F32 = ShaderVariant(
    name="aten_embedding_f32",
    family="aten",
    contract=ShaderContract(
        class_name="AtenEmbeddingF32Program",
        shader_name="aten_embedding_f32",
        fields=(
            TensorFieldSpec("weight", IOKind.INPUT, "weight", TensorContract(dtype="float32", shape=("V", "H"))),
            TensorFieldSpec("indices", IOKind.INPUT, "indices", TensorContract(dtype="int64", shape=("B", "T"))),
...
layout(push_constant) uniform PushConstants { uint T; uint H; uint V; } pc;
...
void main() {
  ...
  output_values[out_index] = (token_id >= 0 && token_id < int64_t(pc.V)) ? weight[uint(token_id) * pc.H + h] : 0.0;
}
""".lstrip(),
)
