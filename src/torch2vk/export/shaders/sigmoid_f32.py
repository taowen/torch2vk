...
void main() {
    ...
    if (idx < pc.N) {
        ...
        output_values[idx] = {{STORE_SIGMOID}};
    }
}
"""


def make_sigmoid_variant(node: Node, activation_dtype: str = "float32") -> ShaderVariant | None:
...
def _source(activation_dtype: str) -> str:
    return (
        ...
        .replace("{{STORE_SIGMOID}}", activation_store("1.0 / (1.0 + exp(-v))", activation_dtype))
    )
