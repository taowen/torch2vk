...
void main() {
    ...
    if (idx < pc.N) {
        float v = x[idx];
        output_values[idx] = 0.5 * v * (1.0 + tanh(0.7978845608 * (v + 0.044715 * v * v * v)));
    }
}
"""


def make_gelu_variant(node: Node) -> ShaderVariant | None:
    return make_unary_elementwise(_SOURCE, "gelu_f32", node)
