...
void main() {
    ...
    if (idx < pc.N) { output_values[idx] = {{STORE_POW}}; }
}
"""


def make_pow_base_scalar_variant(
    node: Node,
    activation_dtype: str = "float32",
) -> ShaderVariant | None:
    if len(node.args) < 2 or not isinstance(node.args[0], (int, float)):
        return None
    base = float(node.args[0])
    exponent = node.args[1]
    if not isinstance(exponent, Node):
        return None
    node.meta["torch2vk_shader_inputs"] = (exponent.name,)
    return make_unary_elementwise(
        _source(base=base, activation_dtype=activation_dtype),
        "pow_base_scalar_f32",
        node,
...
def _source(*, base: float, activation_dtype: str) -> str:
    return (
        ...
        .replace("{{STORE_POW}}", activation_store(f"pow({base:.9g}, float(x[idx]))", activation_dtype))
    )
