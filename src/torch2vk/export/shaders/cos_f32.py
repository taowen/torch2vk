...
void main() {
    ...
    if (idx < pc.N) { output_values[idx] = {{STORE_COS}}; }
}
"""


def make_cos_variant(node: Node, activation_dtype: str = "float32") -> ShaderVariant | None:
...
def _source(activation_dtype: str) -> str:
    return (
        ...
        .replace("{{STORE_COS}}", activation_store("cos(float(x[idx]))", activation_dtype))
    )
