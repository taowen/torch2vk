...
from torch2vk.export.shaders._factory import make_binary_same_shape, node_output_shape, shape_to_contract, flat_numel_expr
...
def make_sub_variant(node: Node) -> ShaderVariant | None:
    out_shape = node_output_shape(node)
    if not out_shape:
        return None

    if len(node.args) >= 2 and not isinstance(node.args[1], Node):
        return _make_sub_scalar(node, out_shape)

    return make_binary_same_shape(_SAME_SOURCE, "sub_f32", node)


def _make_sub_scalar(node: Node, out_shape: tuple[int, ...]) -> ShaderVariant:
    raw = node.args[1]
