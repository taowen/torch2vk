...
void main() {
    ...
    if (idx < pc.N) {
        output_values[idx] = {{STORE_SQRT}};
    }
}
"""


def make_sqrt_variant(node: Node, activation_dtype: str = "float32") -> ShaderVariant | None:
...
def _source(activation_dtype: str) -> str:
    return (
        ...
        .replace(
            "{{STORE_SQRT}}",
            activation_store("sqrt(float(x[idx]))", activation_dtype),
        )
    )
