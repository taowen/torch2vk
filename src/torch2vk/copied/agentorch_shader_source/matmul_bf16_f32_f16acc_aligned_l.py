"""llama.cpp runtime-matched BF16 x F32 cooperative prefill matmul."""
...
from .shader_execution_requirements import ShaderExecutionRequirements
...
#line 1 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp"
...
#line 1 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/types.glsl"
...
#line 32 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp"
...
layout(binding = 0) readonly buffer A { u16vec4 data_a[]; };
...
layout(binding = 1) readonly buffer B { u16vec4 data_b[]; };
...
shared bf16vec2 buf_a[BM * (BK / 2 + 4)];
shared bf16vec2 buf_b[BN * (BK / 2 + 4)];
...
#line 1 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_id_funcs.glsl"
...
#line 138 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp"
#line 1 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm_funcs.glsl"
 void load_a_to_shmem(const uint pos_a, const uint row, const uint col, const uint idx_m, const uint block, const uint end_k) {
            ...
            const uint buf_idx = col * (BK / 2 + 4) + row * 4 / 2;
                       bf16vec4 aa = bf16vec4(uintBitsToBFloat16EXT(data_a[idx]));
            buf_a[buf_idx] = aa.xy;
...
void load_b_to_shmem(const uint pos_b, const uint row, const uint col, const uint idx_n, const uint block, const uint end_k) {
            ...
            const uint buf_idx = col * (BK / 2 + 4) + row * 4 / 2;
                       ...
                       bf16vec4 bb = bf16vec4(uintBitsToBFloat16EXT(data_b[idx]));
...
#line 139 "third_party/llama.cpp/ggml/src/ggml-vulkan/vulkan-shaders/mul_mm.comp"
...
void main() {
    ...
    const uint loadr_a = gl_LocalInvocationID.x % (BK / 4 / 1);
    const uint loadc_a = gl_LocalInvocationID.x / (BK / 4 / 1);
    const uint loadr_b = gl_LocalInvocationID.x % (BK / 4 / 1);
    const uint loadc_b = gl_LocalInvocationID.x / (BK / 4 / 1);

    const uint loadstride_a = gl_WorkGroupSize.x * 4 * 1 / BK;
    const uint loadstride_b = gl_WorkGroupSize.x * 4 * 1 / BK;
    ...
    uint pos_a =
        ...
        batch_idx_a * (p.batch_stride_a / 4) +

        (ir * BM * p.stride_a + start_k) / 4;



    uint pos_b = (batch_idx * p.batch_stride_b + ic * BN * p.stride_b + start_k) / 4;



    coopmat < bfloat16_t, gl_ScopeSubgroup, TM, TK, gl_MatrixUseA > cache_a;
    coopmat < bfloat16_t, gl_ScopeSubgroup, TK, TN, gl_MatrixUseB > cache_b;
    ...
    for (uint block = start_k; block < end_k; block += BK) {
        ...
        [[unroll]] for (uint i = 0; i < BK; i += TK) {
            [[unroll]] for (uint cm_row = 0; cm_row < cms_per_row; cm_row ++) {
                ...
                coopMatLoad(cache_a, buf_a, (warp_r * WM + cm_row * TM) * (BK / 2 + 4) + i / 2, (BK / 2 + 4), gl_CooperativeMatrixLayoutRowMajor);

                [[unroll]] for (uint cm_col = 0; cm_col < cms_per_col; cm_col ++) {
                    coopMatLoad(cache_b, buf_b, (warp_c * WN + cm_col * TN) * (BK / 2 + 4) + i / 2, (BK / 2 + 4), gl_CooperativeMatrixLayoutColumnMajor);
...
def _matmul_k_split(tensors: TensorBindings, _shape_symbols: ShapeSymbols) -> int:
    ...
    if len(out_shape) == 4:
        ...
        return _ceil_div(_ceil_div(in_features, split_k), 256) * 256
...
MATMUL_BF16_F32_F16ACC_ALIGNED_L = shader_variant(
    name="matmul_bf16_f32_f16acc_aligned_l",
    family="matmul_bf16_f32_f16acc_aligned_l",
    execution_requirements=_MATMUL_EXECUTION_REQUIREMENTS,
    contract=shader_contract(
        class_name="MatmulBf16F32F16AccAlignedLProgram",
        shader_name="matmul_bf16_f32_f16acc_aligned_l",
        fields=(
            input_tensor(
                name="weight",
                binding="t_weight",
    ...
    source=_MATMUL_BF16_F32_F16ACC_ALIGNED_CM1_SOURCE,
)

MATMUL_BF16_F32_F16ACC_ALIGNED_L_SPLIT = shader_variant(
    name="matmul_bf16_f32_f16acc_aligned_l_split",
    family="matmul_bf16_f32_f16acc_aligned_l",
    default_output_field="split_k_output",
    execution_requirements=_MATMUL_EXECUTION_REQUIREMENTS,
    contract=shader_contract(
        class_name="MatmulBf16F32F16AccAlignedLSplitProgram",
        shader_name="matmul_bf16_f32_f16acc_aligned_l_split",
        fields=(
            input_tensor(
                name="weight",
                binding="t_weight",
