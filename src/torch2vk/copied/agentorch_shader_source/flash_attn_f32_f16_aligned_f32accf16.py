"""Exact llama.cpp `flash_attn_f32_f16_aligned_f32accf16` shader binding."""
...
from .llama_push_constants import flash_attn_push_constant_block
from .shader_execution_requirements import (
    CooperativeMatrixRequirements,
    ShaderExecutionRequirements,
    SubgroupRequirements,
)
from .shader_variant import ShaderVariantDef, shader_variant
...
_FLASH_ATTN_EXECUTION_REQUIREMENTS = ShaderExecutionRequirements(
...
_FLASH_ATTN_SPECIALIZATION_CONSTANTS = {
    0: 256,
    1: 16,
    2: 64,
    3: 128,
...
FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16 = shader_variant(
    name="flash_attn_f32_f16_aligned_f32accf16",
    family="flash_attn_f32_f16_aligned_f32accf16",
    default_output_field="split_k_output",
    execution_requirements=_FLASH_ATTN_EXECUTION_REQUIREMENTS,
    contract=shader_contract(
        class_name="FlashAttnF32F16AlignedF32AccF16Program",
        shader_name="flash_attn_f32_f16_aligned_f32accf16",
        fields=(
            input_tensor(
                name="q", binding="t_q", role="q", dtypes=("float32",), shape=("B", "S", "QH", "D")
            ),
            input_tensor(
                name="k", binding="t_k", role="k", dtypes=("float16",), shape=("B", "T", "KVH", "D")
            ),
            input_tensor(
                name="v", binding="t_v", role="v", dtypes=("float16",), shape=("B", "T", "KVH", "D")
            ),
            input_tensor(
                name="mask",
                binding="t_mask",
            ...
            input_tensor(
                name="sinks_placeholder",
                binding="t_sinks_placeholder",
                role="sinks_placeholder",
                dtypes=("float32",),
                shape=("B", "S", "QH", "D"),
            ),
            output_tensor(
            ...
            input_tensor(
                name="mask_opt_placeholder",
                binding="t_mask_opt_placeholder",
                role="mask_opt_placeholder",
                dtypes=("float32",),
                shape=("B", "S", "QH", "D"),
            ),
        ),
        uniforms=(),
        push_constants=flash_attn_push_constant_block(
            q_name="q",
            k_name="k",
            v_name="v",
            mask_name="mask",
            output_name="mask_opt_placeholder",
            q_axes=(3, 1, 2, 0),
            k_axes=(3, 1, 2, 0),
            v_axes=(3, 1, 2, 0),
            mask_axes=(3, 2, 1, 0),
        ...
        dispatch=(mul("S", 4), "KVH", "B"),
        tensor_defaults=(
            tensor_binding_default(field_name="sinks_placeholder", source_field_name="q"),
            tensor_binding_default(field_name="mask_opt_placeholder", source_field_name="q"),
        ),
        bindings=(
            storage_buffer_binding(name="t_q", binding=0),
            storage_buffer_binding(name="t_k", binding=1),
            storage_buffer_binding(name="t_v", binding=2),
            storage_buffer_binding(name="t_mask", binding=3),
            storage_buffer_binding(name="t_sinks_placeholder", binding=4),
            storage_buffer_binding(name="t_split_k_output", binding=5),
            storage_buffer_binding(name="t_mask_opt_placeholder", binding=6),
        ),
    ),
    specialization_constants=_FLASH_ATTN_SPECIALIZATION_CONSTANTS,
    compile_defines=_FLASH_ATTN_COMPILE_DEFINES,
    source=_FLASH_ATTN_SOURCE,
)

FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16_DIRECT = shader_variant(
    name="flash_attn_f32_f16_aligned_f32accf16_direct",
    family="flash_attn_f32_f16_aligned_f32accf16",
    execution_requirements=_FLASH_ATTN_EXECUTION_REQUIREMENTS,
    contract=shader_contract(
        class_name="FlashAttnF32F16AlignedF32AccF16DirectProgram",
        shader_name="flash_attn_f32_f16_aligned_f32accf16_direct",
        fields=(
            input_tensor(
                name="q", binding="t_q", role="q", dtypes=("float32",), shape=("B", "S", "QH", "D")
            ),
...
_FLASH_ATTN_SPECIALIZATION_CONSTANTS_CLAMPED = {
    **_FLASH_ATTN_SPECIALIZATION_CONSTANTS,
    5: 1,
}

FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16_DIRECT_CLAMPED = shader_variant(
    name="flash_attn_f32_f16_aligned_f32accf16_direct_clamped",
    family="flash_attn_f32_f16_aligned_f32accf16",
    execution_requirements=_FLASH_ATTN_EXECUTION_REQUIREMENTS,
    contract=shader_contract(
        class_name="FlashAttnF32F16AlignedF32AccF16DirectClampedProgram",
        shader_name="flash_attn_f32_f16_aligned_f32accf16_direct_clamped",
        fields=(
            input_tensor(
                name="q", binding="t_q", role="q", dtypes=("float32",), shape=("B", "S", "QH", "D")
            ),
    ...
    specialization_constants=_FLASH_ATTN_SPECIALIZATION_CONSTANTS_CLAMPED,
    compile_defines=_FLASH_ATTN_COMPILE_DEFINES,
    source=_FLASH_ATTN_SOURCE,
)


def _flash_attn_no_gqa_split_variant(*, name: str, k_num: int) -> ShaderVariantDef:
    return shader_variant(
        ...
        execution_requirements=_FLASH_ATTN_EXECUTION_REQUIREMENTS,
        contract=shader_contract(
            class_name=f"{name.title().replace('_', '')}Program",
            shader_name=name,
            fields=(
                input_tensor(
                    name="q",
            ...
            dispatch=(mul(ceil_div("S", 16), k_num), "QH", "B"),
            tensor_defaults=(
                tensor_binding_default(field_name="sinks_placeholder", source_field_name="q"),
                tensor_binding_default(field_name="mask_opt_placeholder", source_field_name="q"),
            ),
...
FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16_SPLIT_K2 = _flash_attn_no_gqa_split_variant(
    name="flash_attn_f32_f16_aligned_f32accf16_split_k2",
    k_num=2,
)
FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16_SPLIT_K3 = _flash_attn_no_gqa_split_variant(
    name="flash_attn_f32_f16_aligned_f32accf16_split_k3",
    k_num=3,
)
FLASH_ATTN_F32_F16_ALIGNED_F32ACCF16_SPLIT_K4 = _flash_attn_no_gqa_split_variant(
    name="flash_attn_f32_f16_aligned_f32accf16_split_k4",
    k_num=4,
)
