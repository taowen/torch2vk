"""F16 embedding row lookup into F32 sequence outputs."""

from __future__ import annotations

from agentorch.kernel.contract import ceil_div, input_tensor, output_tensor, shader_contract, storage_buffer_binding, uniform_buffer_binding, uniform_ivec4

from .shader_variant import shader_variant


EMBEDDING_LOOKUP_F16_F32_SEQUENCE = shader_variant(
    name="embedding_lookup_f16_f32_sequence",
    family="embedding_lookup_f16_f32",
    contract=shader_contract(
        class_name="EmbeddingLookupF16F32SequenceProgram",
        shader_name="embedding_lookup_f16_f32_sequence",
        fields=(
            input_tensor(name="input_ids", binding="t_input_ids", role="input_ids", dtypes=("int32",), shape=("B", "S")),
            input_tensor(name="weight", binding="t_weight", role="weight", dtypes=("float16",), shape=("V", "H")),
...
void main() {
    ...
    t_output[token_offset * hidden + hidden_index] = float(t_weight[uint(token_id) * hidden + hidden_index]);
}
""".lstrip(),
)
