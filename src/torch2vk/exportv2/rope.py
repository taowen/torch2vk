...
def precompute_qwen3_asr_mrope(
    *,
    ...
    if sum(mrope_section) != half_dim:
        raise ValueError(f"mrope_section {mrope_section} must sum to head_dim/2={half_dim}")
    if position_ids.shape[0] != 3 or position_ids.shape[1] != 1:
        raise ValueError(f"position_ids must have shape (3, 1, seq_len), got {position_ids.shape}")
