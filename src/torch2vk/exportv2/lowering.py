"""Aten-to-shader lowering registry used by exportv2."""
...
class OpLoweringRegistry:
    ...
    def resolve_node(self, node: StaticNode) -> OpShaderBinding | None:
        target, inputs, _outputs = node
        return self.resolve_target_inputs(target=target, inputs=inputs)
...
DEFAULT_LOWERING_REGISTRY = OpLoweringRegistry(
    (
        OpShaderBinding(target="aten.select.int", shader="ATEN_SELECT_INT_I64"),
        OpShaderBinding(
        ...
        OpShaderBinding(target="aten.sum.dim_IntList", shader="ATEN_SUM_DIM1_F32"),
        OpShaderBinding(target="aten.where.self", shader="ATEN_WHERE_F32"),
        OpShaderBinding(
        ...
        OpShaderBinding(target="aten.torch2vk.conv_out.default", shader="CONV_OUT_F32"),
        OpShaderBinding(
        ...
        OpShaderBinding(target="aten.native_layer_norm.default", shader="LAYER_NORM_F32"),
        OpShaderBinding(
        ...
        OpShaderBinding(target="aten.rms_norm.default", shader="TEXT_RMS_NORM_F32"),
        OpShaderBinding(
            target="aten.linear.default",
        ...
        OpShaderBinding(target="aten.torch2vk.text_rope.default", shader="TEXT_ROPE_F32"),
        OpShaderBinding(
        ...
        OpShaderBinding(
            ...
            match=lambda inputs: len(inputs) == 2
            and any(
                inputs[1].endswith(s) for s in ("o_proj", "down_proj", ".o_proj", ".down_proj")
            ),
        ),
        OpShaderBinding(
        ...
        OpShaderBinding(target="aten.torch2vk.rope_table.default", shader="ROPE_TABLE_F32"),
    )
)
