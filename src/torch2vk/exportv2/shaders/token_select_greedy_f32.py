...
TOKEN_SELECT_GREEDY_F32 = ShaderVariant(
    name="token_select_greedy_f32",
    family="text",
    contract=ShaderContract(
        class_name="TokenSelectGreedyF32Program",
        shader_name="token_select_greedy_f32",
        fields=(
            TensorFieldSpec(
                "logits",
                IOKind.INPUT,
                "logits",
                TensorContract(dtype="float32", shape=(1, "T", "V")),
            ),
            TensorFieldSpec(
                "eos_token_ids",
                IOKind.INPUT,
                "eos_token_ids",
                TensorContract(dtype="int64", shape=("E",)),
            ),
            TensorFieldSpec(
                "next_token", IOKind.OUTPUT, "next_token", TensorContract(dtype="int64", shape=(1,))
            ),
            TensorFieldSpec(
                "done", IOKind.OUTPUT, "done", TensorContract(dtype="uint32", shape=(1,))
            ),
        ),
