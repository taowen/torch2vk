...
void main() {
    ...
    const int start = int(t_codebook_offsets[codebook]);
    const int end = (codebook + 1u < codebooks) ? int(t_codebook_offsets[codebook + 1u]) : int(vocab);
...
OMNIVOICE_CODEBOOK_ARGMAX_F32 = ShaderVariant(
    name="omnivoice_codebook_argmax_f32",
    ...
    contract=ShaderContract(
        ...
        bindings=(
            ...
            Binding("logits", 1, BindingAccess.READ),
            Binding("codebook_offsets", 2, BindingAccess.READ),
        ),
        uniforms=(UniformBlock("sizes", 3, ("S", "B", "V", "C")),),
        dispatch=("C", "S", "B"),
        push_constants=None,
    ),
    source=_SOURCE,
