...
class Qwen3ASRThinkerTextRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(self, config, device=None):
        ...
        inv_freq = 1.0 / (
            config.rope_theta ** (torch.arange(0, dim, 2, device=device).float() / dim)
        )
        return inv_freq, 1.0
...
class Qwen3ASRThinkerForConditionalGeneration(Qwen3ASRPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def get_placeholder_mask(
        ...
        if input_ids is None:
            special_audio_mask = (
                inputs_embeds
                == self.get_input_embeddings()(torch.tensor(self.config.audio_token_id, dtype=torch.long, device=inputs_embeds.device))).all(-1)
        else:
